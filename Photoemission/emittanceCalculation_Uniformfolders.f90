...
character(5), dimension(nFolders) :: folderList
integer, dimension(nFolders) :: mpList, neInit
...
folderList=(/ "1e5", "2e5", "5e5", "1e6", "2e6", "5e6", "1e7", "2e7", "6e7", "1e8", "2e8" /)
mpList(:) = 100
mpList(1:3)= 10
neInit=(/ 1e5, 2e5, 5e5, 1e6, 2e6, 5e6, 1e7, 2e7, 6e7, 1e8, 2e8 /)
...
do i=1,size(folderList)
    ...
    if(ierr == 0) then
        ...
        write(41,*) neInit(i),j*mpList(i),rmsEmittance
        write(42,*) neInit(i),j*mpList(i),rmsEmittance(:)/meanP(3)*real(j)
    end if
end do
